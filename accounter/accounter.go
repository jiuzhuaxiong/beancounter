...
func (a *Accounter) processTransactions() {
	for hash, tx := range a.transactions {
		...
		if tx.height > a.maxHeight {
			reporter.GetInstance().Log(fmt.Sprintf("transaction %s has height %d > MAX HEIGHT (%d)", hash, tx.height, a.maxHeight))
			delete(a.transactions, hash)
		}
		// remove transactions which haven't been mined
		if tx.height == 0 {
			reporter.GetInstance().Log(fmt.Sprintf("transaction %s has not been mined, yet (height=0)", hash))
			delete(a.transactions, hash)
		}
	}
